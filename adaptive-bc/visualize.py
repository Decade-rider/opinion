...
LastEditTime: 2024-04-12 19:39:36
...
def plot_opinion_evolution(model, filename):
    ...
    plt.savefig(f'adaptive-bc/data/emotion/300/{filename}.png')
    plt.close()
...
def plot_emotion_extreme_total_evolution(model, filename):
    # 准备数据
    negative_emotions_extreme_total = []
    positive_emotions_extreme_total = []
    for emotions in model.emotion_history:
        emotion_count = Counter(emotions)
        negative_emotions_extreme_total.append(emotion_count['极端负面情绪'] + emotion_count['负面情绪'])
        positive_emotions_extreme_total.append(emotion_count['极端正面情绪'] + emotion_count['正面情绪'])

    # 绘制极端情绪随时间变化的折线图
    plt.figure(figsize=(12, 8))
    plt.plot(negative_emotions_extreme_total, label='负面情绪')
    plt.plot(positive_emotions_extreme_total, label='正面情绪')
    plt.title('情绪随时间的变化')
    plt.xlabel('时间步')
    plt.ylabel('数量')
    plt.legend()
    plt.savefig(f'adaptive-bc/data/emotion/300/{filename}_emotion_total_evolution.png')
    plt.close()
